...
class RestaurantServiceTest {
  ...
  void normalHourServingRadiusIs5Kms() throws IOException {
    ...
    List<Restaurant> expectedRestaurants = loadRestaurantsDuringNormalHours();
    GetRestaurantsResponse getexpectedRestaurantsResponse= new GetRestaurantsResponse(new ArrayList<>(expectedRestaurants));

    when(restaurantRepositoryServiceMock.findAllRestaurantsCloseBy((any(Double.class)),any(Double.class),any(LocalTime.class),any(Double.class))).thenReturn(expectedRestaurants);

    GetRestaurantsRequest getRestaurantsRequest= new GetRestaurantsRequest(20.0, 30.0);

    GetRestaurantsResponse actualresponse= restaurantService.findAllRestaurantsCloseBy(getRestaurantsRequest,LocalTime.of(23, 0));

    assertEquals(getexpectedRestaurantsResponse, actualresponse);
    // verify(restaurantService).findAllRestaurantsCloseBy(eq(request.getLatitude()), eq(request.getLongitude()), eq(currentTime), eq(5.0));
}
